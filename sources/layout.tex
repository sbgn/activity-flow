...
\begin{enumerate}
  \item the overlapping nodes define a glyph (\eg a stacking of \glyph{biological activity} and \glyph{unit of information}).
  \item nodes overlapping compartments  (\eg a biological activity placed on the compartment border).
\end{enumerate}
...
In case of node-edge crossing the edge must be drawn on the top of the node (\fig{af:layout2}). See also recommendation \ref{af:crosEdNo} (crossing between
...
If an activity has all participants in the same compartment the activity node and all edges/arcs have to be in this compartment. If an activity has participants in at least two different compartments, the activity node has to be either in a compartment where the activity has at least one participant or in the empty space.
