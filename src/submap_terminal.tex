...
A \glyph{submap terminal} is a decorator of the \glyph{submap} (\sect{submap}).
It is a named handle, or reference, to both an \glyph{BA} (\sect{af:biologicalActivity}) or \glyph{compartment} (\sect{compartment}) of the map, and a \glyph{tag} (\sect{tag}) of the map the \glyph{submap} glyph refers to.
...
The flat edge opposite to the arrowhead must be aligned to the edge of the \glyph{submap} glyph, and the incoming \glyph{equivalence arc} (\sect{equivalenceArc}) must be linked to its middle.
...
\begin{figure}[H]
  ...
  \caption{The \AF glyph for \glyph{submap terminal}.}
